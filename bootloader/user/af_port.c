...
uint32_t	SPI_IAPBUF[512];
uint32_t af_spi_flash_check(unsigned long addr, unsigned long size)
{
	unsigned int check_sum = 0x0;
	int j,read_size;
	for (j=0; j<size;)
	{
		read_size = ((size-j) > sizeof(SPI_IAPBUF)) ? sizeof(SPI_IAPBUF) : (size-j);
		dcd_port_read(addr+j, (u32 *)SPI_IAPBUF, read_size);
		check_sum = af_calc_crc32(check_sum, SPI_IAPBUF, read_size);
		j+=read_size;
	}
	return check_sum;
}

AfErrCode af_spi_erase_upgrade_data_partition(void)
{
	return (AfErrCode)dcd_port_erase(UPGRADEaddr, UPGRADE_DATA_SIZE);
}
#endif
